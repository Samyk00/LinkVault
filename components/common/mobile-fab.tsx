...
export function MobileFAB() {
  ...
  return (
    <Button
      size="lg"
      className="fixed bottom-6 right-6 z-fixed bottom-6 right-6 z-50 size-14 rounded-full shadow-lg md:hidden transition-shadow duration-200 hover:shadow-xltAddLinkModalOpen(true)}
      aria-label="Add new link"
    >
      <Plus className="h-6 w-6" />
      <span classsize-6r-only">Add Link</span>
    </Button>
  );
}
